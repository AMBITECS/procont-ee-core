...
void startPstorage()
{
    ...
    {
        sprintf((char*)log_msg, "Creating Persistent Storage file\n");
        log(log_msg);
    }
    ...
    if (ps == NULL)
    {
        sprintf((char*)log_msg, "Persistent Storage: Error creating persistent memory file!\n");
        log(log_msg);
        //return 0;
    }
    ...
    {
        sprintf((char*)log_msg, "Persistent Storage: Error writing to persistent memory file!\n");
        log(log_msg);
        //return 0;
    }
    fclose(ps);
...
int readPersistentStorage()
{
	...
	if (fd == NULL)
	{
        sprintf((char*)log_msg, "Warning: Persistent Storage file not found\n");
        log(log_msg);
		return 0;
	}
	...
	{
        sprintf((char*)log_msg, "Persistent Storage: Error while trying to read persistent.file!\n");
        log(log_msg);
		return 0;
	}
	fclose(fd);
    
    sprintf((char*)log_msg, "Persistent Storage: Reading persistent.file into local buffers\n");
    log(log_msg);
