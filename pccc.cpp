...
uint16_t Command_Protocol(pccc_header header, unsigned char *buffer, int buffer_size)
{
	...
	{	
		return Protected_Logical_Read_Reply(header,buffer,buffer_size);
	}
	...
	{	
		return Protected_Logical_Write_Reply(header,buffer,buffer_size);
	}
	else
	{
		...
		sprintf((char*)log_msg, "PCCC: Unsupportedd Command/Data Function Code!\n");
		log(log_msg); 
		return -1;  //return length as -1 to signify that the CMD Code/Function Code was not recognize
	}
}
...
uint16_t Protected_Logical_Read_Reply(pccc_header header, unsigned char *buffer, int buffer_size)
{
	...
	unsigned int len_resp = 4 + buffer[5];
    /*check if the message is long enough- Left in for future error handling setup*/
    // if (buffer_size < 8) // PCCC Error Handling; Make sure that the buffer size is at least 8

	// Read Coils
	if (    buffer[6] == PCCC_FN_OUTPUT &&
	        buffer[7] == PCCC_OUTPUT_LOGICAL_SLOT)  Pccc_ReadCoils(buffer, buffer_size);

	// Read Discrete Inputs
	else if( buffer[6] == PCCC_FN_INPUT &&
	         buffer[7] == PCCC_INPUT_LOGICAL_SLOT)  Pccc_ReadDiscreteInputs(buffer, buffer_size);

	// Read Holding Registers[PURE, 16Bit Mem, 32bit MEM]
	else if((buffer[6] == PCCC_FN_INT  || buffer[6] == PCCC_FN_FLOAT)  &&
	        (buffer[7] == PCCC_INTEGER || buffer[7] == PCCC_FLOATING_POINT))
	                                                Pccc_ReadHoldingRegisters(buffer, buffer_size);
	else
	{
		unsigned char log_msg[1000];
		unsigned char *p = log_msg;
		sprintf((char*)log_msg, "PCCC: Error occured while processing Protected Logical Read\n");
		log(log_msg); 
		return -1; //return length as -1 to signify that the CMD Code/Function Code was not recognize
	}
...
{	
	...
	// Write Coil
	if(buffer[6] == PCCC_FN_OUTPUT && buffer[7] == PCCC_OUTPUT_LOGICAL_SLOT) Pccc_WriteCoil(buffer, buffer_size);

	// Write Register
	else if( (buffer[6] == PCCC_FN_FLOAT || buffer[6] == PCCC_FN_INT) &&
	         (buffer[7] == PCCC_INTEGER  || buffer[7] == PCCC_FLOATING_POINT)) Pccc_WriteRegister(buffer, buffer_size);

	// Function Code Error
	else /*Left in for future error handling setup*/
	{
		// PCCC Error Handling; Make sure that the buffer size is at least 8.
		// If none of the defined File Type and File Numbers match, error unrecognized File Type and File Number.
	}
	return len_resp;	
}
...
void Pccc_ReadCoils(unsigned char *buffer, int buffer_size) //Working QX Read
{
	...
	/*if (buffer_size < 10)
	{
		// PCCC Error Handling (Fill in?);
	    // This Request must have at least 10 bytes. If it doesn't, its a corrupted message
	}*/

    //Start based on the Element and Subelemnt values in the Command Packet
	Start = word_pccc(buffer[8],buffer[9]);
    //Save the byte size or byte data length to the variable from the command packet
	Mask = log2( word_pccc(buffer[10],buffer[11]) );
	...
	for (int i = 0; i < ByteDataLength; i++)
	{
		for(int j = 0; j < 8; j++)
		{
			...
			{
				// PCCC Error Handling (Fill in?);
				// If the position is greater than the MAX COILS, ERROR Overflow?
			}
		}
	}
	pthread_mutex_unlock(&bufferLock);
	/*Left in for future error handling setup*/
	// if (pccc_error != ERR_NONE) //PCCC Error Handling (Fill in?); Deetermine if there was an error:
}
...
//-----------------------------------------------------------------------------
 void Pccc_WriteCoil(unsigned char *buffer, int buffer_size) //QX Write NEEDS WRITE MULTIPLE
 {
	...
	*/
	...
	{
		unsigned char value;
		if (word_pccc(buffer[10 + mask_offset],buffer[11 + mask_offset]) > 0) value = 1;
		else value = 0;

		pthread_mutex_lock(&bufferLock);
		if (bool_output[Start][Mask] != NULL) *bool_output[Start][Mask] = value;
		pthread_mutex_unlock(&bufferLock);
	}
}
