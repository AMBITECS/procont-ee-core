...
{	
		...
		if ( header->command[0] == 0x65/*65*/ || header->command[0] == 0x70 )
		{
		...
		else if ( enipDataUnknown->item1_id[0] == 0x81 )
		{
            ...
            // Unconnected and Connected type data that is accounted for to check enipType
            // This means the labels "item1_data and item2_length
            // do not contain what is stated but what it would be for the correct type
		{
			// PCCC type 2 - Unconnected Data Item
			return 2;
		}																								  
		else if ( enipDataUnknown->item1_data[0] == 0xb2 &&
		            ( (enipDataUnknown->item2_length[1]==0x54) || //0x54 opens connection
		              (enipDataUnknown->item2_length[1]==0x4e)    //0x4e closes connection
		            )
		        )
		{
...
{	
	return ((uint16_t)header->length[1] << 8) | (uint16_t)header->length[0];
}
...
{	
	return ((uint16_t)enipDataUnknown->item2_length[1] << 8) | (uint16_t)enipDataUnknown->item2_length[0];
}
...
{	
	return ((uint16_t)enipDataUnconnected->item2_length[1] << 8) | (uint16_t)enipDataUnconnected->item2_length[0];
}
...
{	
	return ((uint16_t)enipDataConnected_0x70->item2_length[1] << 8) | (uint16_t)enipDataConnected_0x70->item2_length[0];
}
...
//-----------------------------------------------------------------------------  
int parseEnipHeader(    unsigned char * const buffer, int buffer_size,
                        struct enip_header *header/*, struct enip_data_Unknown *enipDataUnknown*/)
{	
    //verify if message is big enough
    if (buffer_size < ENIP_MIN_LENGTH) return -1;
    ...
    if (buffer_size - 24 < enip_data_size) return -1;
...
// ENIP Type: UNKNOWN
//-----------------------------------------------------------------------------
void parseEnipUnknown(unsigned char * const buffer, struct enip_data_Unknown *enipDataUnknown)
{
...
// ENIP Type: Unconnected
//-----------------------------------------------------------------------------
void parseEnipUnconnected( unsigned char * const buffer,
...
// ENIP Type: Connected
//-----------------------------------------------------------------------------
void parseEnipConnected(unsigned char * const buffer, struct enip_data_Connected *enipDataConnected)
{
...
void parseEnipDataConnected_0x70(unsigned char * const buffer, struct enip_data_Connected_0x70 *enipDataConnected_0x70)
{
...
// Command Code: 0x65
//-----------------------------------------------------------------------------  
int registerEnipSession(const struct enip_header *header)
{	
    unsigned char r[4];
                ...
                struct enip_data_Connected *enipDataConnected )
{
	...
	{	
		...
		// return new PCCC data size
		uint16_t newPcccSize = processPCCCMessage(pcccData, currentPcccSize);
		if (newPcccSize == -1) return -1;	//error in PCCC.cpp
	...
	else return -1; // log error to openPLC
}
...
int sendUnitData(struct enip_header *header, struct enip_data_Connected_0x70 *enipDataConnected_0x70)
{
	...
	int write_check = 0;
	int write_check_2 = 0;
	if (pcccData[4] == 0xab) masked_write_check = pcccData[5]; //Byte Size

	if (pcccData[4] == 0xaa) {
		//write_check = 2;
		write_check = 2;
	}
	if (pcccData[4] == 0xa2 && pcccData[7] == 0x8a) write_check_2 = -6;
	// if(pcccData[4] == 0xa2 && pcccData[7] == 0x89) write_check_2 = -2;
    ...
    // Use to be '+ 16' uint16_t newHeaderSize = newPcccSize + newItem2Size + 16;
	uint16_t newHeaderSize = newPcccSize + newItem2Size + 14 + masked_write_check + write_check + write_check_2;
...
{	
    ...
    if (parseEnipHeader(buffer, buffer_size, &header/*, &enipDataUnknown*/) < 0) return -1;

	parseEnipUnknown(buffer, &enipDataUnknown);

	// Register a Session
    if (header.command[0] == 0x65) return registerEnipSession(&header);
	if (header.command[0] == 0x70)
	{
        // Send Unit Data ---> works with Connected Type
	...
	if (enipType == 2)      parseEnipUnconnected(buffer, &enipDataUnconnected);
	else if (enipType == 3) parseEnipConnected(buffer, &enipDataConnected);
	else if (enipType < 0)
	{
		// log UNKNOWN Enip Type message to open plc 
		sprintf((char*)log_msg, "ENIP: Received unsupported EtherNet/IP Type\n");
		log(log_msg);
	}
	
	//writeDataContents(&enipDataUnknown);
    //if (header.command[0] == 0x65) return registerEnipSession(&header); // Register a Session
    ...
    {
        p += sprintf((char*)p, "Unknown EtherNet/IP request: ");
        for (int i = 0; i < buffer_size; i++)
        {
            p += sprintf((char*)p, "%02x ", (unsigned char)buffer[i]);
        }
        p += sprintf((char*)p, "\n");
        log(log_msg);
        return -1;
